...
export class CuponCodeService {
  ...
  async getAllCuponCodes():Promise<CuponCode[]> {
    const cupon_codes = await this.cuponCodeRepo.findAll({include: {all: true}});
    return cupon_codes;
  }

  async getCuponCodeById(id:number) {
    const cupon_code = await this.cuponCodeRepo.findOne({where: {id}, include: {all: true}});
    return cupon_code;
  }

  async updateCuponCodeById(id:number, updateCuponCodeDto:UpdateCuponCodeDto):Promise<CuponCode> {
    const cupon_code = await this.cuponCodeRepo.update(updateCuponCodeDto, {where: {id}, returning: true});
