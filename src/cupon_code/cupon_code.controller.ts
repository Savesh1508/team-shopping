...
export class CuponCodeController {
  ...
  @ApiResponse({status: 200, description: 'Cupon code by Id', type: [CuponCode]})
  @Get(':id')
  ...
  @ApiResponse({status: 200, description: 'Updated cupon code', type: [CuponCode]})
  @Put(':id')
  async updateCuponCodeById(@Param('id') id:string, @Body() updateComanyDto: UpdateCuponCodeDto):Promise<CuponCode>{
  ...
  @ApiResponse({status: 200, description: 'Deleted cupon code', type: [CuponCode]})
  @Delete(':id')
